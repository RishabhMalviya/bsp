...
class ReplayBuffer:
    ...
    def sample(self, batch_size: int, H: int = 1) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        if H == 1:
            idx = np.random.choice(self.size, size=batch_size, replace=False)
        else:
            if H > self.size:
                raise ValueError(f"Sequence length H={H} exceeds buffer size {self.size}")
            ...
            if self.size < self.capacity:
                starts = np.arange(self.size - H + 1)
            else:
                starts = (self.ptr + np.arange(self.capacity - H + 1)) % self.capacity
            ...
            break_idx = (starts[:, None] + np.arange(H - 1)[None, :]) % self.capacity
            invalid = (self.terminated[break_idx] > 0) | (self.truncated[break_idx] > 0)
            valid_starts = starts[~invalid.any(axis=1)]

            if len(valid_starts) < batch_size:
                raise ValueError(
                    f"Only {len(valid_starts)} valid length-{H} sequences available, "
                    f"requested batch_size={batch_size}"
                )

            chosen = np.random.choice(valid_starts, size=batch_size, replace=False)
            idx = (chosen[:, None] + np.arange(H)[None, :]) % self.capacity
        ...
        return (
            ...
            torch.from_numpy(self.terminated[idx]).to(self.device),
            torch.from_numpy(self.truncated[idx]).to(self.device),
        )
